...
class NRF24L01:
    ...
    def reg_read_multiple(self, reg, len):
        bytes = bytearray(len)
        self.cs(0)
        self.spi.readinto(bytes, reg)
        self.spi.readinto(bytes)
        self.cs(1)
        return bytes
    ...
    def send_done(self):
        ...
        if status & MAX_RT:
            print(f"Failed because of MAX_RT")
        return 1 if status & TX_DS else 2

    def print_info(self):
        config = self.reg_read(CONFIG)
        en_aa = self.reg_read(EN_AA)
        en_rxaddr = self.reg_read(EN_RXADDR)
        setup_aw = self.reg_read(SETUP_AW)
        setup_retr = self.reg_read(SETUP_RETR)
        rf_ch = self.reg_read(RF_CH)
        rf_setup = self.reg_read(RF_SETUP)
        status = self.reg_read(STATUS)
        observe_tx = self.reg_read(OBSERVE_TX)
        cd = self.reg_read(CD)

        rx_addr_p0 = self.reg_read_multiple(RX_ADDR_P0, 5)
        rx_addr_p1 = self.reg_read_multiple(RX_ADDR_P1, 5)
        rx_addr_p2 = self.reg_read(RX_ADDR_P2)
        rx_addr_p3 = self.reg_read(RX_ADDR_P3)
        rx_addr_p4 = self.reg_read(RX_ADDR_P4)
        rx_addr_p5 = self.reg_read(RX_ADDR_P5)
        tx_addr = self.reg_read_multiple(TX_ADDR, 5)
        
        rx_pw_p0 = self.reg_read(RX_PW_P0)
        rx_pw_p1 = self.reg_read(RX_PW_P1)
        rx_pw_p2 = self.reg_read(RX_PW_P2)
        rx_pw_p3 = self.reg_read(RX_PW_P3)
        rx_pw_p4 = self.reg_read(RX_PW_P4)
        rx_pw_p5 = self.reg_read(RX_PW_P5)

        fifo_status = self.reg_read(FIFO_STATUS)
        dynpd = self.reg_read(DYNPD)

        print("================ Print All ================\n" )
        print(f"CONFIG: {config:#04x}")
        print(f"EN_AA: {en_aa:#04x}")
        print(f"EN_RXADDR: {en_rxaddr:#04x}")
        print(f"SETUP_AW: {setup_aw:#04x}")
        print(f"SETUP_RETR: {setup_retr:#04x}")
        print(f"RF_CH: {rf_ch:#04x}")
        print(f"RF_SETUP: {rf_setup:#04x}")
        print(f"STATUS: {status:#04x}")
        print(f"OBSERVE_TX: {observe_tx:#04x}")
        print(f"CD: {cd:#04x}")
        print(f"RX_ADDR_P0: {rx_addr_p0.hex()}")
        print(f"RX_ADDR_P1: {rx_addr_p1.hex()}")
        print(f"RX_ADDR_P2: {rx_addr_p2:#04x}")
        print(f"RX_ADDR_P3: {rx_addr_p3:#04x}")
        print(f"RX_ADDR_P4: {rx_addr_p4:#04x}")
        print(f"RX_ADDR_P5: {rx_addr_p5:#04x}")
        print(f"TX_ADDR: {tx_addr.hex()}")
        print(f"RX_PW_P0: {rx_pw_p0:#04x}")
        print(f"RX_PW_P1: {rx_pw_p1:#04x}")
        print(f"RX_PW_P2: {rx_pw_p2:#04x}")
        print(f"RX_PW_P3: {rx_pw_p3:#04x}")
        print(f"RX_PW_P4: {rx_pw_p4:#04x}")
        print(f"RX_PW_P5: {rx_pw_p5:#04x}")
        print(f"FIFO_STATUS: {fifo_status:#04x}")
        print(f"DYNPD/FEATURE: {dynpd:#04x}")
        print("================ Print All ================\n" )
